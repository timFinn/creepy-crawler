...
class Motor {
	public:
		...
		Motor(int argc, char **argv) {
			  ...
			  ros::NodeHandle node;
			  node.getParam("motor_id", this->motor_id);
			  //ROS_INFO("Param test for motor: %d", motor_id);
			  printf("Param test for motor: %d", this->motor_id);
			  ros::Subscriber sub = node.subscribe("motor_cmd", 5, this->cmdCallback);
		}
		...
		void setSpeed(float speed) {
		  //ROS_INFO("Setting speed of motor %d to %f", motor_id, speed);
		  if (this->motor_id < 0.0 || this->motor_id > 1.0) {
			ROS_ERROR("Invalid motor_id [%d] Expected 0 or 1", this->motor_id);
			return;
		  }
		  if (speed < -255.0 || speed > 255.0) {
			ROS_ERROR("Invalid Speed [%f] Expected >=-255 <=255", speed);
			return;
		  }
		  ...
		  if (speed == 0.0) {
			...
			// FORWARD
			setPin(in2Pin[this->motor_id], 0);
			setPin(in1Pin[this->motor_id], 1);
			setPWM(pwmPin[this->motor_id], 0, speed * 16.0);
		  }
		  else { // => speed < 0
			// BACKWARD
			setPin(in1Pin[this->motor_id], 0);
			setPin(in2Pin[this->motor_id], 1);
			setPWM(pwmPin[this->motor_id], 0, -speed * 16.0);
		  }
		}

		void static turnOffMotors() {
		  for (int i = 0; i < 4; ++i)
			stop(i);
		}

		// Receive a vector4 of int corresponding to the desired motor speeds.
		void static cmdCallback(const std_msgs::Float32::ConstPtr& msg) {
			ROS_INFO("Callback for motor: %d", this->motor_id);
			setSpeed(msg->data);
		}
};
...
int main(int argc, char **argv) {
  // Setup.
  Motor motor = Motor(argc, argv);
  init_hat();
  setPWMFreq(1600);
  atexit(motor.turnOffMotors);
