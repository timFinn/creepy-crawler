...
void setSpeed( float speed) {
  ...
  if (speed == 0.0) {
    ...
    setPWM(pwmPin[motor_id], 0, speed * 16.0);
  }
  else { // => speed < 0
    // BACKWARD
    setPin(in1Pin[motor_id], 0);
...
//	ROS_INFO("Callback for motor: %d", motor_id);
    setSpeed(msg->data);
}
...
int main(int argc, char **argv) {
  ...
  //if(node.getParam("~motor_id", motor_id))
  //{
	  //ROS_INFO("Got param: %d", motor_id);
  //}
  //else
  //{
	  //ROS_INFO("Failed to get param 'motor_id'");
  //}
  ros::param::param<int>("~motor_id", motor_id, 0);
  
//  ROS_INFO("Param test for motor: %d", motor_id);
  ros::Subscriber sub = node.subscribe("motor_cmd", 5, cmdCallback);
